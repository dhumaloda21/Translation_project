...
def bill_button_operation():
    ...
    total_price_str = totalPrice.get()  # e.g. 'Rs. 212 /-'
    total_price_str = total_price_str.replace('/-', '')  # e.g. 'Rs. 212 '
    total_price_str = total_price_str.replace('Rs. ', '')  # e.g. '212 '
    total_price1 = float(total_price_str)  # e.g. 212.0
    ...
    with open(f"Bill Records\\{folder}\\{customer_name+customer_contact}.txt", "rb") as f:
        file_content = f.read()        
    
    
    
    sql = "INSERT INTO customers (name, contact, total_bill , bill_file) VALUES (%s, %s, %s, %s)"
    val = (customer_name, customer_contact, total_price1, file_content)
